<template>
  <v-container fluid>
    <v-row>
      <v-col cols="12" sm="12" md="6" lg="4" v-for="(product, index) in products" :key="index">
        <v-card> 
          <a :href="product.url">
            <v-img height="200" :src="product.image" alt="Placeholder" />
          ...
          <v-list dense>
            <v-item-group>
              <v-list-item role="listitem" v-for="(cannabinoid, index) in product.cannabinoids" :key="index">
            ...
            <v-item-group>
              <v-list-item role="listitem" v-for="(terpene, index) in product.terpenes" :key="index">
                  <div>{{ terpene.pct }}</div>
