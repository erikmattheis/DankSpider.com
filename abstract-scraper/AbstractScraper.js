...
async function scrapePage(url) {
    ...
    while (currentPageUrl) {
        ...
        currentPageUrl = nextPageUrl ? `https://${config.domain}${nextPageUrl}` : null;
    }
...
function extractProduct($, element) {
    ...
    for (const property of selectors.properties)
        $(element).find(property.selector).each((_, propElement) => {
            const value = $(propElement).find(selectors.propertyValue).text().trim();
